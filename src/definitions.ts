...
export interface TcpSocketManagerPlugin {
  ...
  startServer(options: { port: number }): Promise<{ success: boolean }>;
  stopServer(): Promise<{ success: boolean }>;
  receiveMessage(): Promise<{ message: string }>;
  getClientCount(): Promise<{ count: number }>;
  disconnectAllClients(): Promise<{ success: boolean }>;
  ...
  sendMessageToServer(options: { message: string }): Promise<{ success: boolean }>;
